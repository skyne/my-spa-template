...
export default function withToaster(WrappedComponent, key) {
  return class WithToaster extends React.Component {
    constructor(props) {
      ...
      this.connectedToaster = connect((state) => ({ messages: state.toaster[this.wrappedComponentName].messages }), { hideMessage: (k) => hideMessage(this.wrappedComponentName, k) })(Toaster);
